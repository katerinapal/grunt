...
import ext_gruntlegacyutil_util from "grunt-legacy-util";
import * as utiltask_taskjsjs from "./util/task";
...
grunt.util = ext_gruntlegacyutil_util;
grunt.util.task = utiltask_taskjsjs;
...
grunt.tasks = function(tasks, options, done) {
  ...
  if (option('version')) {
    ...
    if (option('verbose')) {
      // --verbose
      verbose.writeln('Install path: ' + ext_path_path.resolve(__dirname, '..'));
  ...
  task.options({
    error: function(e) {
      ...
      if (done) {
        ...
        ext_gruntlegacyutil_util.exit(0);
      }
    }
  });
