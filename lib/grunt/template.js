...
import ext_dateformat_dateformat from "dateformat";
'use strict';

var encapsulated_template;

// The module to be exported.
var template = encapsulated_template = {};

// External libs.
template.date = ext_dateformat_dateformat;
...
template.setDelimiters = function(name) {
  ...
  grunt_gruntjs.util._.extend(grunt_gruntjs.util._.templateSettings, delimiters.lodash);
...
template.process = function(tmpl, options) {
  ...
  try {
    ...
    while (tmpl.indexOf(delimiters.opener) >= 0) {
      tmpl = grunt_gruntjs.util._.template(tmpl, options)(data);
    ...
    if (String(e) === 'SyntaxError: Unexpected token ILLEGAL' && /\n|\r/.test(tmpl)) {
      grunt_gruntjs.log.errorlns('A special character was detected in this template. ' +
    ...
    grunt_gruntjs.warn(e, grunt_gruntjs.fail.code.TEMPLATE_ERROR);
  }
  // Normalize linefeeds and return.
  return grunt_gruntjs.util.normalizelf(tmpl);
};
