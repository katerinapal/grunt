...
var parent = grunt_gruntjs.util.task.create();

var encapsulated_task;

// The module to be exported.
var task = encapsulated_task = Object.create(parent);
...
task.registerTask = function(name) {
  ...
  thisTask.meta = grunt_gruntjs.util._.clone(registry.meta);
  ...
  thisTask.fn = function(arg) {
    ...
    errorcount = grunt_gruntjs.fail.errorcount;
    // Return the number of errors logged during this task.
    Object.defineProperty(this, 'errorCount', {
      enumerable: true,
      get: function() {
        return grunt_gruntjs.fail.errorcount - errorcount;
      }
    });
    // Expose task.requires on `this`.
    this.requires = task.requires.bind(task);
    // Expose config.requires on `this`.
    this.requiresConfig = grunt_gruntjs.config.requires;
    // Return an options object with the specified defaults overwritten by task-
    // specific overrides, via the "options" property.
    this.options = function() {
      var args = [{}].concat(grunt_gruntjs.util.toArray(arguments)).concat([
        grunt_gruntjs.config([name, 'options'])
      ]);
      var options = grunt_gruntjs.util._.extend.apply(null, args);
      grunt_gruntjs.verbose.writeflags(options, 'Options');
      return options;
    };
    ...
    // Actually log.
    grunt_gruntjs[logger].header('Running "' + this.nameArgs + '"' +
      (this.name !== this.nameArgs ? ' (' + this.name + ')' : '') + ' task');
    // If --debug was specified, log the path to this task's source file.
    grunt_gruntjs[logger].debug('Task source: ' + thisTask.meta.filepath);
...
task.normalizeMultiTaskFiles = function(data, target) {
  var prop, obj;
  var files = [];
  if (grunt_gruntjs.util.kindOf(data) === 'object') {
    if ('src' in data || 'dest' in data) {
      ...
      files.push(obj);
    } else if (grunt_gruntjs.util.kindOf(data.files) === 'object') {
      for (prop in data.files) {
        files.push({src: data.files[prop], dest: grunt_gruntjs.config.process(prop)});
      }
    } else if (Array.isArray(data.files)) {
      grunt_gruntjs.util._.flattenDeep(data.files).forEach(function(obj) {
        var prop;
        if ('src' in obj || 'dest' in obj) {
          files.push(obj);
        } else {
          for (prop in obj) {
            files.push({src: obj[prop], dest: grunt_gruntjs.config.process(prop)});
          }
        }
      });
    }
  } else {
    files.push({src: data, dest: grunt_gruntjs.config.process(target)});
  }

  // If no src/dest or files were specified, return an empty files array.
  if (files.length === 0) {
    grunt_gruntjs.verbose.writeln('File: ' + '[no files]'.yellow);
    return [];
  }

  // Process all normalized file objects.
  files = grunt_gruntjs.util._(files).chain().forEach(function(obj) {
    if (!('src' in obj) || !obj.src) { return; }
    // Normalize .src properties to flattened array.
    if (Array.isArray(obj.src)) {
      obj.src = grunt_gruntjs.util._.flatten(obj.src);
    } else {
      obj.src = [obj.src];
    }
  }).map(function(obj) {
    // Build options object, removing unwanted properties.
    var expandOptions = grunt_gruntjs.util._.extend({}, obj);
    delete expandOptions.src;
    delete expandOptions.dest;

    // Expand file mappings.
    if (obj.expand) {
      return grunt_gruntjs.file.expandMapping(obj.src, obj.dest, expandOptions).map(function(mapObj) {
        // Copy obj properties to result.
        var result = grunt_gruntjs.util._.extend({}, obj);
        // Make a clone of the orig obj available.
        result.orig = grunt_gruntjs.util._.extend({}, obj);
        // Set .src and .dest, processing both as templates.
        result.src = grunt_gruntjs.config.process(mapObj.src);
        result.dest = grunt_gruntjs.config.process(mapObj.dest);
      ...
      Object.defineProperty(result, 'src', {
        ...
        get: function fn() {
          ...
          if (!('result' in fn)) {
            ...
            src = Array.isArray(src) ? grunt_gruntjs.util._.flatten(src) : [src];
            // Expand src files, memoizing result.
            fn.result = grunt_gruntjs.file.expand(expandOptions, src);
          }
          return fn.result;
        }
      });
  ...
  if (grunt_gruntjs.option('verbose')) {
    files.forEach(function(obj) {
      var output = [];
      if ('src' in obj) {
        output.push(obj.src.length > 0 ? grunt_gruntjs.log.wordlist(obj.src) : '[no src]'.yellow);
      }
      if ('dest' in obj) {
        output.push('-> ' + (obj.dest ? String(obj.dest).cyan : '[no dest]'.yellow));
      }
      if (output.length > 0) {
        grunt_gruntjs.verbose.writeln('Files: ' + output.join(' '));
      }
    });
  }
...
task.registerMultiTask = function(name, info, fn) {
  ...
  task.registerTask(name, info, function(target) {
    ...
    this.args = grunt_gruntjs.util.toArray(arguments).slice(1);
    ...
    this.options = function() {
      var targetObj = grunt_gruntjs.config([name, target]);
      var args = [{}].concat(grunt_gruntjs.util.toArray(arguments)).concat([
        grunt_gruntjs.config([name, 'options']),
        grunt_gruntjs.util.kindOf(targetObj) === 'object' ? targetObj.options : {}
      ]);
      var options = grunt_gruntjs.util._.extend.apply(null, args);
      grunt_gruntjs.verbose.writeflags(options, 'Options');
      return options;
    ...
    this.data = grunt_gruntjs.config([name, target]);
    ...
    Object.defineProperty(this, 'filesSrc', {
      enumerable: true,
      get: function() {
        return grunt_gruntjs.util._(this.files).chain().map('src').flatten().uniq().value();
      }.bind(this)
    });
...
task.renameTask = function(oldname, newname) {
  ...
  try {
    ...
    grunt_gruntjs.log.error(e.message);
  }
};
...
task.runAllTargets = function(taskname, args) {
  ...
  var targets = Object.keys(grunt_gruntjs.config.getRaw(taskname) || {});
  ...
  if (targets.length === 0) {
    grunt_gruntjs.log.error('No "' + taskname + '" targets found.');
    return false;
  }
...
function loadTask(filepath) {
  ...
  var filename = ext_path_path.basename(filepath);
  var msg = 'Loading "' + filename + '" tasks...';
  var regCount = 0;
  var fn;
  try {
    // Load taskfile.
    fn = require(ext_path_path.resolve(filepath));
    if (typeof fn === 'function') {
      fn.call(grunt_gruntjs, grunt_gruntjs);
    }
    grunt_gruntjs.verbose.write(msg).ok();
    // Log registered/renamed/unregistered tasks.
    ['un', ''].forEach(function(prefix) {
      var list = grunt_gruntjs.util._.chain(registry[prefix + 'tasks']).uniq().sort().value();
      if (list.length > 0) {
        regCount++;
        grunt_gruntjs.verbose.writeln((prefix ? '- ' : '+ ') + grunt_gruntjs.log.wordlist(list));
      }
    });
    if (regCount === 0) {
      grunt_gruntjs.verbose.warn('No tasks were registered or unregistered.');
    }
  } catch (e) {
    // Something went wrong.
    grunt_gruntjs.log.write(msg).error().verbose.error(e.stack).or.error(e);
  }
  // Restore registry.
...
function loadTasksMessage(info) {
  ...
  grunt_gruntjs.verbose.subhead('Registering ' + info + ' tasks.');
}

// Load tasks and handlers from a given directory.
function loadTasks(tasksdir) {
  try {
    var files = grunt_gruntjs.file.glob.sync('*.{js,coffee}', {cwd: tasksdir, maxDepth: 1});
    // Load tasks from files.
    files.forEach(function(filename) {
      loadTask(ext_path_path.join(tasksdir, filename));
    });
  } catch (e) {
    grunt_gruntjs.log.verbose.error(e.stack).or.error(e);
  }
}
...
task.loadTasks = function(tasksdir) {
  ...
  if (grunt_gruntjs.file.exists(tasksdir)) {
    loadTasks(tasksdir);
  } else {
    grunt_gruntjs.log.error('Tasks directory "' + tasksdir + '" not found.');
  }
};
...
task.loadNpmTasks = function(name) {
  ...
  var root = ext_path_path.resolve('node_modules');
  var pkgfile = ext_path_path.join(root, name, 'package.json');
  var pkg = grunt_gruntjs.file.exists(pkgfile) ? grunt_gruntjs.file.readJSON(pkgfile) : {keywords: []};
  ...
  if (pkg.keywords && pkg.keywords.indexOf('gruntcollection') !== -1) {
    ...
    Object.keys(pkg.dependencies).forEach(function(depName) {
      ...
      var filepath = grunt_gruntjs.file.findup('node_modules/' + depName, {
        cwd: ext_path_path.resolve('node_modules', name),
        nocase: true
      });
      if (filepath) {
        // Load this task plugin recursively.
        task.loadNpmTasks(ext_path_path.relative(root, filepath));
      }
    });
    loadTaskDepth--;
    return;
  ...
  var tasksdir = ext_path_path.join(root, name, 'tasks');
  if (grunt_gruntjs.file.exists(tasksdir)) {
    loadTasks(tasksdir);
  } else {
    grunt_gruntjs.log.error('Local Npm module "' + name + '" not found. Is it installed?');
  }
};
...
task.init = function(tasks, options) {
  ...
  if (allInit || options.gruntfile === false) {
    ...
    gruntfile = grunt_gruntjs.option('gruntfile') ||
      grunt_gruntjs.file.findup('Gruntfile.{js,coffee}', {nocase: true});
    msg = 'Reading "' + (gruntfile ? ext_path_path.basename(gruntfile) : '???') + '" Gruntfile...';
  }

  if (options.gruntfile === false) {
    // Grunt was run as a lib with {gruntfile: false}.
  } else if (gruntfile && grunt_gruntjs.file.exists(gruntfile)) {
    grunt_gruntjs.verbose.writeln().write(msg).ok();
    // Change working directory so that all paths are relative to the
    // Gruntfile's location (or the --base option, if specified).
    process.chdir(grunt_gruntjs.option('base') || ext_path_path.dirname(gruntfile));
  ...
  } else if (grunt_gruntjs.option('gruntfile')) {
    // If --config override was specified and it doesn't exist, complain.
    grunt_gruntjs.log.writeln().write(msg).error();
    grunt_gruntjs.fatal('Unable to find "' + gruntfile + '" Gruntfile.', grunt_gruntjs.fail.code.MISSING_GRUNTFILE);
  } else if (!grunt_gruntjs.option('help')) {
    grunt_gruntjs.verbose.writeln().write(msg).error();
    grunt_gruntjs.log.writelns(
    ...
    grunt_gruntjs.fatal('Unable to find Gruntfile.', grunt_gruntjs.fail.code.MISSING_GRUNTFILE);
  }

  // Load all user-specified --npm tasks.
  (grunt_gruntjs.option('npm') || []).map(String).forEach(task.loadNpmTasks);
  // Load all user-specified --tasks.
  (grunt_gruntjs.option('tasks') || []).map(String).forEach(task.loadTasks);
};
